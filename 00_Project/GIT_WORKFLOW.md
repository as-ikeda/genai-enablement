...
作業ブランチは、Pull Requestが`main`へマージされたら削除する。
...
| --- | --- | --- |
| `main` | 削除しない | 安定版として残す |
| `docs/*` | 削除する | ガイド・手順書の変更はmainに取り込まれた時点で役目を終える |
| `study/*` | 削除する | 学習ログはmainに残り、作業ブランチは不要になる |
| `practice/*` | 削除する | 実務活用例はmainに残り、作業ブランチは不要になる |
| `fix/*` | 削除する | 修正内容はmainに反映済みのため不要になる |

### 削除する理由

- マージ済みの変更は`main`の履歴に残る
- Pull Requestにも変更内容と会話が残る
- 古いブランチを残すと、作業中か完了済みか分かりづらくなる
- ブランチ一覧をきれいに保つと、初心者でも状況を把握しやすい

### GitHub上での削除

Pull Requestのマージ後に表示される`Delete branch`を押す。

この操作で削除されるのはリモートの作業ブランチであり、`main`に取り込まれた内容は消えない。

### ローカル側の整理

GitHubでブランチを削除した後、ローカルでは以下を実行する。
...
`git branch -d` は安全削除。未マージのブランチであれば削除を止めてくれる。

### 例外

以下のような場合は、すぐ削除せず一時的に残してもよい。
